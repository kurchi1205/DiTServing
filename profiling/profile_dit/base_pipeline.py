...
from transformer_models import Transformer2DModelWithAnnotations
from diffusers import DiTPipeline, DiffusionPipeline, DiTTransformer2DModel, AutoencoderKL, ImagePipelineOutput, DDIMScheduler
...
class DitPipeline(DiTPipeline):
    def __init__(
        self,
        transformer: Transformer2DModelWithAnnotations,
        vae: AutoencoderKL,
        scheduler: DDIMScheduler,
        id2label: Optional[Dict[int, str]] = None,
    ):
        new_transformer = copy.deepcopy(transformer)
        new_transformer.__class__ = Transformer2DModelWithAnnotations
        new_transformer.__init__(**transformer.config)
        new_transformer.load_state_dict(transformer.state_dict())
        new_transformer = new_transformer.half()
    ...
    def __call__(
        ...
        for t in self.progress_bar(self.scheduler.timesteps):
            ...
            tr_rng = nvtx.start_range(message=f"Noise Prediction at timestep {t}", color="blue")
