...
def process_each_timestep(handler, request_id, request_pool):
    ...
    if request_pool.requests[request_id]["current_timestep"] == 0:
        noise_scaled, sigmas, conditioning, neg_cond, seed_num = handler.prepare_for_first_timestep(width, height, prompt, seed_type="rand")
    ...
    latent = handler.denoise_each_step(denoiser, model, noise_scaled, sigmas[current_timestep], sigmas[current_timestep - 1], sigmas[current_timestep + 1], extra_args)
