...
intervals = sorted({int(interval) for scores in lpips_data.values() for interval in scores})
intervals_str = [str(i) for i in intervals]

# Plot each prompt's FID vs cache interval
plt.figure(figsize=(12, 6))

for prompt_name, scores in lpips_data.items():
...
plt.title("LPIPS Score vs Cache Interval (per prompt)")
